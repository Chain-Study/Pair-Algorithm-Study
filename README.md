...
- 매주 화, 금 오후 9시 ~ 오후 11시
- 하루에 한 문제를 1시간 30분 안에 푸는 것을 목표로 한다.
    - 9시 : 시작
    - ~ 9시 40분: 페어 문제 풀이
    - ~ 9시 50분: 휴식 + 잡담
    - ~ 10시 30분: 종료
- GitHub Organization 기능을 활용하여 Repository를 Fork하여 PR로 기록을 남긴다.
- 개인사정 불참시 정해진 시간의 1시간 전까지 사전 공지를 원칙으로 한다.
- 풀 문제를 스터디 전에 번갈아 [문제보관함](https://www.notion.so/redniche/200975cde215475fa8b3f9f82242fe79?v=3f2b97428e1a4547a10ecd15d8e692d7)에 등록한다.
- 마지막 20분 동안 문제를 리뷰하고 못 풀었을 경우 해답을 같이 확인한다.
...
| 시간 | 내용 |
| --- | --- |
| 21:00 ~ 21:40 | 알고리즘 문제 풀이 |
| 21:40 ~ 21:50 | 휴식을 취하지만 스터디의 집중에 방해가 될 행동은 알아서 삼가한다 |
| 21:50 ~ 22:10 | 마지막 풀이시간 |
| 22:10 ~ 22:30 | 풀이 리뷰, 해답 확인 |
...
- 커밋 규칙
    ...
    - 풀이
        ...
        - 파일 추가
            - `add: 성명/풀이날짜_플랫폼_문제이름`
            - ex) `add: 장예찬/220912_pg_N개의최소공배수`
        - 파일 수정
            - `fix: 성명/풀이날짜_플랫폼_문제이름`
        - 파일 이동
            - `move: /경로.../성명/풀이날짜_플랫폼_문제이름 to /새경로.../성명/풀이날짜_플랫폼_문제이름`
        - 파일명 변경
            - `rename: 성명/풀이날짜_플랫폼_문제이름`
        - 플랫폼 이름
...
`/1층폴더명/2층폴더명/성명/풀이날짜_플랫폼_문제이름.py` 

`/1층폴더명/성명/풀이날짜_플랫폼_문제이름.py`

ex) `/graph/dfs/장예찬/220912_boj_DFS스페셜저지.py`   `/graph/dfs/고하람/220912_boj_DFS스페셜저지.py`
...
| brute force | combination | 완전 탐색 중 조합을 이용한 풀이 |
|  | permutation | 완전 탐색 중 순열을 이용한 풀이 |
| data struct | hash | 해시 자료구조를 이용한 풀이 |
...
| graph | bfs | 그래프의 완전탐색 중 bfs 를 사용한 풀이 |
